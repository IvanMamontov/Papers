...
\begin{document}
	...
	\begin{frame}
 	...
 	\end{frame}
  	...
  	\end{frame}
  	\begin{frame}
  		\frametitle{What is vint encoding?}
  		To encode the decimal number $128_{10}=1000\:0000_2$:
		\vskip15pt 
  		\begin{tikzpicture}[
 		      	start chain = going right,
     			node distance = 0pt,
				ArrayStyle/.style={draw, minimum width=3.5em, minimum 									height=2em, outer sep=0pt, on chain}]
			\node [ArrayStyle, minimum width=8em] (1) {1 0 0 0 0 0 0 0};
			\draw[decorate,decoration={brace, amplitude=10pt, 					raise=5pt, mirror}]
  				(1.south west) to node[black,midway,below= 15pt] 				{$8$ bits} (1.south east);
		\end{tikzpicture} 
  	\end{frame}
  	\begin{frame}
  		\frametitle{What is vint encoding?}
		...
		\vskip15pt 
  		\begin{tikzpicture}[
     			...
     			node distance = 0pt,
				ArrayStyle/.style={draw, minimum width=3.5em, minimum 									height=2em, outer sep=0pt, on chain}]
			\node [ArrayStyle, minimum width=8em] (1) {1};
			\node [ArrayStyle, minimum width=8em] (2) {0 0 0 0 0 0 0};			
			\draw[decorate,decoration={brace, amplitude=10pt, 					raise=5pt, mirror}]
  				(1.south west) to node[black,midway,below= 15pt] 				{$1$ bits} (1.south east);
  			\draw[decorate,decoration={brace, amplitude=10pt, 					raise=5pt, mirror}]
  				(2.south west) to node[black,midway,below= 15pt] 				{$7$ bits} (2.south east);
		\end{tikzpicture} 
  	\end{frame}
  	\begin{frame}
  		\frametitle{What is vint encoding?}
		...
		\vskip15pt	
  		\begin{tikzpicture}[
     			...
     			node distance = 0pt,
				ArrayStyle/.style={draw, minimum width=3.5em, minimum 									height=2em, outer sep=0pt, on chain}]
			\node [ArrayStyle, minimum width=8em] (1) {0 0 0 0 0 0 1};
			\node [ArrayStyle, minimum width=8em] (2) {0 0 0 0 0 0 0};
			\draw[decorate,decoration={brace, amplitude=10pt, 					raise=5pt, mirror}]
  				(1.south west) to node[black,midway,below= 15pt] 				{$7$ bits} (1.south east);
  			\draw[decorate,decoration={brace, amplitude=10pt, 					raise=5pt, mirror}]
  				(2.south west) to node[black,midway,below= 15pt] 				{$7$ bits} (2.south east);		
		\end{tikzpicture} 
  	\end{frame}  	
  	\begin{frame}
		...
		\vskip15pt 
  		\begin{tikzpicture}[
			...
			\node [ArrayStyle, minimum width=8em] (2) {0 0 0 0 0 0 1};
			\node [ArrayStyle, xshift=1em] (3) {{\color{red!100}0}};
			\node [ArrayStyle, minimum width=8em] (4) {0 0 0 0 0 0 0};
			\draw[decorate,decoration={brace, amplitude=10pt, 					raise=5pt, mirror}]
  	...
  	\begin{frame}
		...
		\end{tikzpicture} 
		\vskip15pt 	
		vint requires 2 bytes instead of 4 bytes!
  	\end{frame}
  	\begin{frame}
  		\frametitle{What is vint encoding?}
  		\begin{itemize}
    		    ...
    		    \item cons
    		    \begin{itemize}
    		    		\item requires statement on every byte% which is very costly since the CPU cannot easily predict the branch outcome.
    		    \end{itemize}
		\end{itemize}
  	\end{frame}
  	\begin{frame}
...
for (int i = 0; i < 4; i++) { 
 c[i] = a[i] + b[i];     
		...
		\end{verbatim}
		\begin{itemize}
			\item JIT $\approx96$ machine instructions
			\item gcc $\approx32$ machine scalar instructions
